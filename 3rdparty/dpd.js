...
(function (undefined) {
  ...
  var root = "http://www.thepadmenu.com";
...
dpd.translate = dpd("/translate");
dpd.translate.on = function(ev, fn) {
  return dpd.on("translate" + ":" + ev, fn);
}
dpd.translate.once = function(ev, fn) {
  return dpd.once("translate" + ":" + ev, fn);
}
dpd.translate.off = function(ev, fn) {
  return dpd.off("translate" + ":" + ev, fn);
}
dpd.translate.rename = function (path, body, fn) {
  dpd.translate.exec("rename", path, body, fn);
}

dpd.s3bucket = dpd("/s3bucket");
dpd.s3bucket.on = function(ev, fn) {
  return dpd.on("s3bucket" + ":" + ev, fn);
}
dpd.s3bucket.once = function(ev, fn) {
  return dpd.once("s3bucket" + ":" + ev, fn);
}
dpd.s3bucket.off = function(ev, fn) {
  return dpd.off("s3bucket" + ":" + ev, fn);
}

dpd.systranslate = dpd("/systranslate");
dpd.systranslate.on = function(ev, fn) {
  return dpd.on("systranslate" + ":" + ev, fn);
}
dpd.systranslate.once = function(ev, fn) {
  return dpd.once("systranslate" + ":" + ev, fn);
}
dpd.systranslate.off = function(ev, fn) {
  return dpd.off("systranslate" + ":" + ev, fn);
}
dpd.systranslate.rename = function (path, body, fn) {
  dpd.systranslate.exec("rename", path, body, fn);
}

dpd.cafe = dpd("/cafe");
dpd.cafe.on = function(ev, fn) {
  return dpd.on("cafe" + ":" + ev, fn);
}
dpd.cafe.once = function(ev, fn) {
  return dpd.once("cafe" + ":" + ev, fn);
}
dpd.cafe.off = function(ev, fn) {
  return dpd.off("cafe" + ":" + ev, fn);
}
dpd.cafe.rename = function (path, body, fn) {
  dpd.cafe.exec("rename", path, body, fn);
}

dpd.menu = dpd("/menu");
dpd.menu.on = function(ev, fn) {
  return dpd.on("menu" + ":" + ev, fn);
}
dpd.menu.once = function(ev, fn) {
  return dpd.once("menu" + ":" + ev, fn);
}
dpd.menu.off = function(ev, fn) {
  return dpd.off("menu" + ":" + ev, fn);
}
dpd.menu.rename = function (path, body, fn) {
  dpd.menu.exec("rename", path, body, fn);
}

dpd.limit = dpd("/limit");
dpd.limit.on = function(ev, fn) {
  return dpd.on("limit" + ":" + ev, fn);
}
dpd.limit.once = function(ev, fn) {
  return dpd.once("limit" + ":" + ev, fn);
}
dpd.limit.off = function(ev, fn) {
  return dpd.off("limit" + ":" + ev, fn);
}
dpd.limit.rename = function (path, body, fn) {
  dpd.limit.exec("rename", path, body, fn);
}

dpd.menugenerator = dpd("/menugenerator");
dpd.menugenerator.on = function(ev, fn) {
  return dpd.on("menugenerator" + ":" + ev, fn);
}
dpd.menugenerator.once = function(ev, fn) {
  return dpd.once("menugenerator" + ":" + ev, fn);
}
dpd.menugenerator.off = function(ev, fn) {
  return dpd.off("menugenerator" + ":" + ev, fn);
}

dpd.pricetree = dpd("/pricetree");
dpd.pricetree.on = function(ev, fn) {
  return dpd.on("pricetree" + ":" + ev, fn);
}
dpd.pricetree.once = function(ev, fn) {
  return dpd.once("pricetree" + ":" + ev, fn);
}
dpd.pricetree.off = function(ev, fn) {
  return dpd.off("pricetree" + ":" + ev, fn);
}
dpd.pricetree.rename = function (path, body, fn) {
  dpd.pricetree.exec("rename", path, body, fn);
}

dpd.recoverhash = dpd("/recoverhash");
dpd.recoverhash.on = function(ev, fn) {
  return dpd.on("recoverhash" + ":" + ev, fn);
}
dpd.recoverhash.once = function(ev, fn) {
  return dpd.once("recoverhash" + ":" + ev, fn);
}
dpd.recoverhash.off = function(ev, fn) {
  return dpd.off("recoverhash" + ":" + ev, fn);
}
dpd.recoverhash.rename = function (path, body, fn) {
  dpd.recoverhash.exec("rename", path, body, fn);
}

dpd.order = dpd("/order");
dpd.order.on = function(ev, fn) {
  return dpd.on("order" + ":" + ev, fn);
}
dpd.order.once = function(ev, fn) {
  return dpd.once("order" + ":" + ev, fn);
}
dpd.order.off = function(ev, fn) {
  return dpd.off("order" + ":" + ev, fn);
}

dpd.regemail = dpd("/regemail");
dpd.regemail.on = function(ev, fn) {
  return dpd.on("regemail" + ":" + ev, fn);
}
dpd.regemail.once = function(ev, fn) {
  return dpd.once("regemail" + ":" + ev, fn);
}
dpd.regemail.off = function(ev, fn) {
  return dpd.off("regemail" + ":" + ev, fn);
}

dpd.device = dpd("/device");
dpd.device.on = function(ev, fn) {
  return dpd.on("device" + ":" + ev, fn);
}
dpd.device.once = function(ev, fn) {
  return dpd.once("device" + ":" + ev, fn);
}
dpd.device.off = function(ev, fn) {
  return dpd.off("device" + ":" + ev, fn);
}
dpd.device.rename = function (path, body, fn) {
  dpd.device.exec("rename", path, body, fn);
}

dpd.file = dpd("/file");
dpd.file.on = function(ev, fn) {
  return dpd.on("file" + ":" + ev, fn);
}
dpd.file.once = function(ev, fn) {
  return dpd.once("file" + ":" + ev, fn);
}
dpd.file.off = function(ev, fn) {
  return dpd.off("file" + ":" + ev, fn);
}
dpd.file.rename = function (path, body, fn) {
  dpd.file.exec("rename", path, body, fn);
}

dpd.cropimage = dpd("/cropimage");
dpd.cropimage.on = function(ev, fn) {
  return dpd.on("cropimage" + ":" + ev, fn);
}
dpd.cropimage.once = function(ev, fn) {
  return dpd.once("cropimage" + ":" + ev, fn);
}
dpd.cropimage.off = function(ev, fn) {
  return dpd.off("cropimage" + ":" + ev, fn);
}

dpd.tariffhistory = dpd("/tariff-history");
dpd.tariffhistory.on = function(ev, fn) {
  return dpd.on("tariff-history" + ":" + ev, fn);
}
dpd.tariffhistory.once = function(ev, fn) {
  return dpd.once("tariff-history" + ":" + ev, fn);
}
dpd.tariffhistory.off = function(ev, fn) {
  return dpd.off("tariff-history" + ":" + ev, fn);
}
dpd.tariffhistory.rename = function (path, body, fn) {
  dpd.tariffhistory.exec("rename", path, body, fn);
}

dpd.option = dpd("/option");
dpd.option.on = function(ev, fn) {
  return dpd.on("option" + ":" + ev, fn);
}
dpd.option.once = function(ev, fn) {
  return dpd.once("option" + ":" + ev, fn);
}
dpd.option.off = function(ev, fn) {
  return dpd.off("option" + ":" + ev, fn);
}
dpd.option.rename = function (path, body, fn) {
  dpd.option.exec("rename", path, body, fn);
}

dpd.menutype = dpd("/menutype");
dpd.menutype.on = function(ev, fn) {
  return dpd.on("menutype" + ":" + ev, fn);
}
dpd.menutype.once = function(ev, fn) {
  return dpd.once("menutype" + ":" + ev, fn);
}
dpd.menutype.off = function(ev, fn) {
  return dpd.off("menutype" + ":" + ev, fn);
}
dpd.menutype.rename = function (path, body, fn) {
  dpd.menutype.exec("rename", path, body, fn);
}

dpd.program = dpd("/program");
dpd.program.on = function(ev, fn) {
  return dpd.on("program" + ":" + ev, fn);
}
dpd.program.once = function(ev, fn) {
  return dpd.once("program" + ":" + ev, fn);
}
dpd.program.off = function(ev, fn) {
  return dpd.off("program" + ":" + ev, fn);
}
dpd.program.rename = function (path, body, fn) {
  dpd.program.exec("rename", path, body, fn);
}

dpd.syslanguage = dpd("/syslanguage");
dpd.syslanguage.on = function(ev, fn) {
  return dpd.on("syslanguage" + ":" + ev, fn);
}
dpd.syslanguage.once = function(ev, fn) {
  return dpd.once("syslanguage" + ":" + ev, fn);
}
dpd.syslanguage.off = function(ev, fn) {
  return dpd.off("syslanguage" + ":" + ev, fn);
}
dpd.syslanguage.rename = function (path, body, fn) {
  dpd.syslanguage.exec("rename", path, body, fn);
}

dpd.theme = dpd("/theme");
dpd.theme.on = function(ev, fn) {
  return dpd.on("theme" + ":" + ev, fn);
}
dpd.theme.once = function(ev, fn) {
  return dpd.once("theme" + ":" + ev, fn);
}
dpd.theme.off = function(ev, fn) {
  return dpd.off("theme" + ":" + ev, fn);
}
dpd.theme.rename = function (path, body, fn) {
  dpd.theme.exec("rename", path, body, fn);
}

dpd.recoverpass = dpd("/recoverpass");
dpd.recoverpass.on = function(ev, fn) {
  return dpd.on("recoverpass" + ":" + ev, fn);
}
dpd.recoverpass.once = function(ev, fn) {
  return dpd.once("recoverpass" + ":" + ev, fn);
}
dpd.recoverpass.off = function(ev, fn) {
  return dpd.off("recoverpass" + ":" + ev, fn);
}

dpd.price = dpd("/price");
dpd.price.on = function(ev, fn) {
  return dpd.on("price" + ":" + ev, fn);
}
dpd.price.once = function(ev, fn) {
  return dpd.once("price" + ":" + ev, fn);
}
dpd.price.off = function(ev, fn) {
  return dpd.off("price" + ":" + ev, fn);
}
dpd.price.rename = function (path, body, fn) {
  dpd.price.exec("rename", path, body, fn);
}

dpd.template = dpd("/template");
dpd.template.on = function(ev, fn) {
  return dpd.on("template" + ":" + ev, fn);
}
dpd.template.once = function(ev, fn) {
  return dpd.once("template" + ":" + ev, fn);
}
dpd.template.off = function(ev, fn) {
  return dpd.off("template" + ":" + ev, fn);
}
dpd.template.rename = function (path, body, fn) {
  dpd.template.exec("rename", path, body, fn);
}
...
dpd.tariff = dpd("/tariff");
dpd.tariff.on = function(ev, fn) {
  return dpd.on("tariff" + ":" + ev, fn);
}
dpd.tariff.once = function(ev, fn) {
  return dpd.once("tariff" + ":" + ev, fn);
}
dpd.tariff.off = function(ev, fn) {
  return dpd.off("tariff" + ":" + ev, fn);
}
dpd.tariff.rename = function (path, body, fn) {
  dpd.tariff.exec("rename", path, body, fn);
}

dpd.recoveremail = dpd("/recoveremail");
dpd.recoveremail.on = function(ev, fn) {
  return dpd.on("recoveremail" + ":" + ev, fn);
}
dpd.recoveremail.once = function(ev, fn) {
  return dpd.once("recoveremail" + ":" + ev, fn);
}
dpd.recoveremail.off = function(ev, fn) {
  return dpd.off("recoveremail" + ":" + ev, fn);
}

dpd.language = dpd("/language");
dpd.language.on = function(ev, fn) {
  return dpd.on("language" + ":" + ev, fn);
}
dpd.language.once = function(ev, fn) {
  return dpd.once("language" + ":" + ev, fn);
}
dpd.language.off = function(ev, fn) {
  return dpd.off("language" + ":" + ev, fn);
}
dpd.language.rename = function (path, body, fn) {
  dpd.language.exec("rename", path, body, fn);
}
